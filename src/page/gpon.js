...
const Gpon = () => {
    ...
    const controlGpon = async (
        ...
        const newLine = <TerminalOutput> {res.detail.map((item) => (item))}</TerminalOutput>;
    ...
    const handleRun = async () => {
        try {
            ...
            if (radioValue !== "sync_password") {
                if (form2Values.card === undefined || form2Values.port === undefined || form2Values.onuId === undefined) {
